...
export class Handler {
  ...
  private sender: WsSender
  private uploadsInterval: ReturnType<typeof setInterval> | null = null
  ...
  cancelTasks(): void {
    if (this.uploadsInterval) {
      clearInterval(this.uploadsInterval)
  ...
  async subscribeBatch(batchid: number): Promise<void> {
    await this.safe('subscribeBatch', async () => {
      if (this.uploadsInterval) clearInterval(this.uploadsInterval)
    ...
    await this.safe('createCategory', async () => {
      ...
      if (wikidataQid) {
        try {
          ...
          const claims = [...existingClaims, newClaim]
          const sitelinks = {
  ...
  private startUploadStream(batchid: number): ReturnType<typeof setInterval> {
