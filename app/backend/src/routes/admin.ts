...
const _noopStore = {
  async get(_k: string): Promise<null> { return null },
...
export function createAdminRoutes(dal: AdminDal) {
  return new Elysia({ prefix: '/api/admin' })
    ...
    .get('/upload_requests', async ({ query }) => {
      const offset = ((query.page ?? 1) - 1) * (query.limit ?? 100)
      ...
      const [items, total] = await Promise.all([
        dal.uploads.getAllUploadRequests({ offset, limit: query.limit ?? 100, filterText: query.filter_text, statuses, dateFrom, dateTo }),
        dal.uploads.countAllUploadRequests({ filterText: query.filter_text, statuses, dateFrom, dateTo }),
      ])
      return { items, total }
    }, {
      query: t.Object({
    ...
    .post('/upload_requests/bulk-cancel', async ({ body }) => {
    ...
    .post('/upload_requests/bulk-fail', async ({ body }) => {
    ...
    .get('/presets', async ({ query }) => {
      const offset = ((query.page ?? 1) - 1) * (query.limit ?? 100)
      const [items, total] = await Promise.all([
        dal.presets.getAllPresets({ offset, limit: query.limit ?? 100, filterText: query.filter_text }),
    ...
    .get('/failed_uploads', async ({ query }) => {
    ...
    .put('/upload_requests/:id', async ({ params, body, set }) => {
      const ok = await dal.uploads.updateUploadFields(Number(params.id), body)
      if (!ok) { set.status = 404; return { message: 'Not found' } }
      return { message: 'Upload request updated successfully' }
    }, {
      body: t.Record(t.String(), t.Any()),
    })

    .post('/retry', async ({ body, session, set }) => {
      const tokenPair = session.access_token
      if (!tokenPair) { set.status = 401; return { message: 'No access token in session' } }
